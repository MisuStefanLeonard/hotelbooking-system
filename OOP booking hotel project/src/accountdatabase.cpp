    ...
    int AccountDataBase::Register(const char* FILENAME){
        ...
        while(exit_id_flag == true && exit_id_flag2 == true){
            ...
            if(std::regex_match(id , checkID))
                ...
                std::cout << "Name must be in the range of 16 character and it cannot contain special characters.Please enter another ID ---> ";
            if(pass_map.find(id) == pass_map.end())
                exit_id_flag2 = false;
            else
                std::cout << "Name already exists. Please choose another name " << std::endl;
        }
        std::cout << "---------------------------------------------------------" << std::endl;
