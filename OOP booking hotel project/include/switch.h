...
class SWITCH{
public:
    void Add_Customer(std::vector<Customer>& customer , const char *FILENAME , string curr_id);
    ...
    void Modify_Customer(LinkBetweenAccountAndCustomer LBAAC,std::vector<Customer> &customer,const char*FILENAME , std::unordered_map<string , Customer>& mapofcustomers , string currID);
